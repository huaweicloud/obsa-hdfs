...
public class SequenceFileDataGeneratorSource extends RichParallelSourceFunction<Tuple2<LongWritable, Text>> implements CheckpointedFunction {
     ...
     */
    public SequenceFileDataGeneratorSource(ParameterTool params) {
    ...
    @Override
    public void run(SourceContext<Tuple2<LongWritable, Text>> ctx) throws Exception {
            ...
            if (recordsPerSecond == -1) {
                if (isRunning && isRecordValid() && isTimeValid()) {
                    synchronized (ctx.getCheckpointLock()) {
                        recordsCounter++;
                        ctx.collect(SequenceDataGenerator.getRecord(recordSize));
                    }
                } else {
                    return;
                }
