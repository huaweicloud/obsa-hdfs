...
public class DataGeneratorSource extends RichParallelSourceFunction<String> implements CheckpointedFunction {
     ...
     */
    public DataGeneratorSource(ParameterTool params) {
            ...
            if (recordsPerSecond == -1) {
                if (isRunning && isRecordValid() && isTimeValid()) {
                    synchronized (ctx.getCheckpointLock()) {
                        recordsCounter++;
                        ctx.collect(DataGenerator.getRecord(recordSize));
                    }
                } else {
                    return;
                }
