 ...
 */
class MockMetricsConsumer implements BasicMetricsConsumer {

    /**
     * Class logger.
     */
    private static final Logger LOG = LoggerFactory.getLogger(
        MockMetricsConsumer.class);
     ...
     */
    MockMetricsConsumer(final URI uriName,
        final Configuration configuration) {
        this.uri = uriName;
        this.conf = configuration;
        LOG.info("MockMetricsConsumer with URI [{}] and "
    ...
    public boolean putMetrics(MetricRecord metricRecord) {
        if (metricRecord.getKind().equals(MetricKind.normal)) {
            ...
            setMetricNormalInfo(metricRecord);
        }else {
            ...
            setMetricAbnormalInfo(metricRecord);
        }
        return true;
    }

    private synchronized void setMetricAbnormalInfo(MetricRecord metricRecord) {
        mr = metricRecord;
    }

    private synchronized void setMetricNormalInfo(MetricRecord metricRecord) {
        mr = metricRecord;
    }

    public MetricRecord getMr() {
        return mr;
    }
