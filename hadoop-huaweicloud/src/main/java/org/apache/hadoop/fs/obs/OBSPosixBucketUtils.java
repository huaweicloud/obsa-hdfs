...
final class OBSPosixBucketUtils {
    ...
    static boolean innerFsRenameWithRetry(final OBSFileSystem owner,
        final Path src,
        ...
        do {
            boolean isRegularDirPath =
                newSrcKey.endsWith("/") && newDstKey.endsWith("/");
            try {
                LOG.debug("rename: {}-st rename from [{}] to [{}] ...",
                    retryTime, newSrcKey, newDstKey);
                ...
                try {
                    FileStatus srcFileStatus =
                        OBSCommonUtils.innerGetFileStatusWithRetry(owner, src);
                    if (srcFileStatus.isDirectory()) {
                        newSrcKey = OBSCommonUtils.maybeAddTrailingSlash(
                            newSrcKey);
                        newDstKey = OBSCommonUtils.maybeAddTrailingSlash(
                            newDstKey);
        ...
        LOG.error(
            "rename: failed {}-st rename src [{}] to dest [{}]",
            retryTime, src, dst, lastException);
        throw lastException;
    }
    ...
    static void fsRenameToNewObject(final OBSFileSystem owner,
        ...
        if (!renameBasedOnPosix(owner, OBSCommonUtils.keyToPath(newSrcKey),
            OBSCommonUtils.keyToPath(newdstKey))) {
            throw new IOException(
                "failed to rename " + newSrcKey + "to" + newdstKey);
        }
    }
    ...
    private static void trashFolderIfNeed(final OBSFileSystem owner,
        final String key) throws ObsException, IOException {
        if (needToTrash(owner, key)) {
            mkTrash(owner, key);
            StringBuilder sb = new StringBuilder(owner.getTrashDir());
            ...
            if (!renameBasedOnPosix(owner, OBSCommonUtils.keyToPath(srcKey),
                OBSCommonUtils.keyToPath(dstKey))) {
                throw new IOException(
                    "failed to rename " + srcKey + "to" + dstKey);
            }
            LOG.debug("Moved: '" + key + "' to trash at: " + sb.toString());
        } else {
    ...
    private static boolean needToTrash(final OBSFileSystem owner,
        ...
        if (owner.isEnableTrash()) {
            String trashPathKey = OBSCommonUtils.pathToKey(owner,
                new Path(owner.getTrashDir()));
    ...
    static void fsCreateFolder(final OBSFileSystem owner,
        ...
        while (System.currentTimeMillis() - startTime
            ...
            try {
                ...
                LOG.debug("Failed to create folder [{}], retry time [{}], "
                    + "exception [{}]", newObjectName, retryTime, e);

                IOException ioException = OBSCommonUtils.translateException(
                    "innerFsCreateFolder", newObjectName, e);
    ...
    static void innerFsTruncateWithRetry(final OBSFileSystem owner,
        final Path f, final long newLength)
        throws IOException {
        long delayMs;
        int retryTime = 0;
        long startTime = System.currentTimeMillis();
        while (System.currentTimeMillis() - startTime
            ...
            try {
                ...
                OBSFileSystem.LOG.debug("Failed to truncate [{}] to newLength"
                        + " [{}], retry time [{}], exception [{}]", f,
                    newLength, retryTime, e);
    ...
    private static void innerFsTruncate(final OBSFileSystem owner,
        final Path f, final long newLength) throws IOException {
        LOG.debug("truncate {} to newLength {}", f, newLength);

        try {
            String key = OBSCommonUtils.pathToKey(owner, f);
            owner.getObsClient()
                .truncateObject(owner.getBucket(), key, newLength);
            owner.getSchemeStatistics().incrementWriteOps(1);
        } catch (ObsException e) {
