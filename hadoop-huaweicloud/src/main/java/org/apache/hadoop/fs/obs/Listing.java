...
public class Listing {
   ...
   * @param acceptor the class/predicate to decide which entries to accept in the listing based on
   *     the full file status.
   * @return the iterator
  ...
  /**
   * Interface to implement by the logic deciding whether to accept a summary entry or path as a
  ...
  interface FileStatusAcceptor {
     ...
     * @return true if the entry is accepted (i.e. that a status entry should be generated.
     */
  ...
  /**
   * A remote iterator which only iterates over a single `LocatedFileStatus` value.
   *
   * <p>If the status value is null, the iterator declares that it has no data. This iterator is
   * used to handle {@link OBSFileSystem#listStatus(Path)} calls where the path handed in refers to
   * a file, not a directory: this is the iterator returned.
   */
  static final class SingleStatusRemoteIterator implements RemoteIterator<LocatedFileStatus> {
     ...
     * Constructor.
     *
     * @param status status value: may be null, in which case the iterator is empty.
     */
     ...
     * {@inheritDoc}
     *
     * @return true if there is a file status to return: this is always false for the second
     *     iteration, and may be false for the first.
     ...
     * {@inheritDoc}
     *
     * @return the non-null status element passed in when the instance was constructed, if it ha not
     *     already been retrieved.
     * @throws IOException never
     * @throws NoSuchElementException if this is the second call, or it is the first call and a null
     *     {@link LocatedFileStatus} entry was passed to the constructor.
     */
    @Override
  ...
  /**
   * Accept all entries except the base path and those which map to OBS pseudo directory markers.
   */
  static class AcceptFilesOnly implements FileStatusAcceptor {
    ...
    /**
     * Reject a summary entry if the key path is the qualified Path, or it ends with {@code
     * "_$folder$"}.
     *
  ...
  static class AcceptAllButSelfAndS3nDirs implements FileStatusAcceptor {
    ...
    @Override
    public boolean accept(Path keyPath, ObsObject summary) {
      return !keyPath.equals(qualifiedPath) && !summary.getObjectKey().endsWith(OBS_FOLDER_SUFFIX);
    }
  ...
  /**
   * Wraps up object listing into a remote iterator which will ask for more listing data if needed.
   *
   * <p>This is a complex operation, especially the process to determine if there are more entries
   * remaining. If there are no more results remaining in the (filtered) results of the current
   * listing request, then another request is made <i>and those results filtered</i> before the
   * iterator can declare that there is more data available.
   *
   * <p>The need to filter the results precludes the iterator from simply declaring that if the
   * {@link ObjectListingIterator#hasNext()} is true then there are more results. Instead the next
   * batch of results must be retrieved and filtered.
   *
   * <p>What does this mean? It means that remote requests to retrieve new batches of object
   * listings are made in the {@link #hasNext()} call; the {@link #next()} call simply returns the
   * filtered results of the last listing processed. However, do note that {@link #next()} calls
   * {@link #hasNext()} during its operation. This is critical to ensure that a listing obtained
   * through a sequence of {@link #next()} will complete with the same set of results as a classic
  ...
  class FileStatusListingIterator implements RemoteIterator<FileStatus> {
    ...
    FileStatusListingIterator(
        ObjectListingIterator source, PathFilter filter, FileStatusAcceptor acceptor)
        throws IOException {
      this.source = source;
      this.filter = filter;
    ...
    /**
     * Report whether or not there is new data available. If there is data in the local filtered
     * list, return true. Else: request more data util that condition is met, or there is no more
     * remote listing data.
     *
    ...
    /**
     * Try to retrieve another batch. Note that for the initial batch, {@link ObjectListingIterator}
     * does not generate a request; it simply returns the initial set.
     *
    ...
    private boolean buildNextStatusBatch(ObjectListing objects) {
      ...
      List<FileStatus> stats =
          new ArrayList<>(objects.getObjects().size() + objects.getCommonPrefixes().size());
      // objects
      for (ObsObject summary : objects.getObjects()) {
        ...
        if (acceptor.accept(keyPath, summary) && filter.accept(keyPath)) {
          FileStatus status =
              createFileStatus(
                  keyPath, summary, owner.getDefaultBlockSize(keyPath), owner.getUsername());
      ...
      for (String prefix : objects.getCommonPrefixes()) {
        ...
        if (acceptor.accept(keyPath, prefix) && filter.accept(keyPath)) {
          FileStatus status = new OBSFileStatus(false, keyPath, owner.getUsername());
  ...
  /**
   * Wraps up OBS `ListObjects` requests in a remote iterator which will ask for more listing data
   * if needed.
   *
   * <p>That is:
   *
   * <p>1. The first invocation of the {@link #next()} call will return the results of the first
   * request, the one created during the construction of the instance.
   *
   * <p>2. Second and later invocations will continue the ongoing listing, calling {@link #
   * OBSFileSystem#continueListObjects(ObjectListing)} to request the next batch of results.
   *
   * <p>3. The {@link #hasNext()} predicate returns true for the initial call, where {@link #next()}
   * will return the initial results. It declares that it has future results iff the last executed
  ...
  class ObjectListingIterator implements RemoteIterator<ObjectListing> {
     ...
     * remote IO. For later requests, OBS will be queried, hence the calls may block or fail.
     *
  ...
  /**
   * Take a remote iterator over a set of {@link FileStatus} instances and return a remote iterator
   * of {@link LocatedFileStatus} instances.
   */
