...
class OBSFsDFSListing extends ObjectListing {
    ...
    static String fsDFSListNextBatch(final OBSFileSystem owner,
        ...
        int resultNum = fetchListResultLocally(resultQueue,
    ...
    static int handleOneLevelListTaskResult(final Queue<ListEntity> resultQueue,
        ...
        for (int i = 0; i < oneLevelObjectListings.size(); i++) {
            ...
            for (ObsObject extenedCommonPrefixes
                : oneLevelObjectListing.getExtenedCommonPrefixes()) {
                if (extenedCommonPrefixes.getObjectKey().equals(
                ...
                LOG.debug(
                    "common prefix: " + extenedCommonPrefixes.getObjectKey());
                ...
                if (newResultNum < maxKeyNum) {
                    ...
                    resultQueue.add(
                        new ListEntity(extenedCommonPrefixes, levels.get(i)));
                }
            }
    ...
    static int fetchListResultLocally(final Queue<ListEntity> resultQueue,
        final int maxKeyNum,
        final List<ObsObject> objectSummaries,
        final List<LevelStats> levelStatsList) {
        int resultNum = 0;
        while (!resultQueue.isEmpty() && resultNum < maxKeyNum) {
            ...
            if (listEntity.getType() == ListEntityType.LIST_TAIL) {
                ...
                throw new RuntimeException(
                    "cannot put common prefix (" + listEntity
                        + ") into result queue");
            } else {
