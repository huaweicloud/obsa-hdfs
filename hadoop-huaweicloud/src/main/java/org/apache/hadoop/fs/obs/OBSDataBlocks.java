...
/**
 * Set of classes to support output streaming into blocks which are then uploaded as to OBS as a
 * single PUT, or as part of a multipart request.
 */
final class OBSDataBlocks {
  ...
  /**
   * Validate args to a write command. These are the same validation checks expected for any
   * implementation of {@code OutputStream.write()}.
   *
  ...
  /**
   * The output information for an upload. It can be one of a file or an input stream. When closed,
   * any stream is closed. Any source file is untouched.
   */
  static final class BlockUploadData implements Closeable {
    ...
    /**
     * Get the raw upload stream, if the object was created with one.
     *
  ...
  abstract static class BlockFactory implements Closeable {
     ...
     *
     * @throws IOException Inherited exception; implementations should avoid raising it.
     */
    @Override
    public void close() throws IOException {}
  ...
  abstract static class DataBlock implements Closeable {
     ...
     */
    protected final synchronized void enterState(DestState current, DestState next)
    ...
    /**
     * Predicate to verify that the block has the capacity to write the given set of bytes.
     *
    ...
    /**
     * Write a series of bytes from the buffer, from the offset. Returns the number of bytes
     * written. Only valid in the state {@code Writing}. Base class verifies the state but does no
     * writing.
     *
     * @param buffer buffer
     * @param offset offset
    ...
    int write(byte[] buffer, int offset, int length) throws IOException {
      ...
      Preconditions.checkArgument(
          !(buffer.length - offset < length), "buffer shorter than amount of data to write");
      return 0;
    }
    ...
    /**
     * Switch to the upload state and return a stream for uploading. Base class calls {@link
     * #enterState(DestState, DestState)} to manage the state machine.
     *
     * @return the stream
     * @throws IOException trouble
     */
     ...
     *
     * @return true if the class was in any other state, implying that the subclass should do its
     *     close operations
     */
   ...
   *
   * <p>This was taken from {@code OBSBlockOutputStream} and has the same problem which surfaced
   * there: it can consume a lot of heap space proportional to the mismatch between writes to the
   * stream and the JVM-wide upload bandwidth to the OBS endpoint. The memory consumption can be
   * limited by tuning the filesystem settings to restrict the number of queued/active uploads.
   */
  ...
  /** Stream via Direct ByteBuffers; these are allocated off heap via {@link DirectBufferPool}. */
  static class ByteBufferBlockFactory extends BlockFactory {
    ...
    @Override
    ByteBufferBlock create(long index, int limit) throws IOException {
    ...
    @Override
    public String toString() {
      return "ByteBufferBlockFactory{" + "buffersOutstanding=" + buffersOutstanding + '}';
    }

    /** A DataBlock which requests a buffer from pool on creation; returns it when it is closed. */
    class ByteBufferBlock extends DataBlock {
      ...
      BlockUploadData startUpload() throws IOException {
        ...
        return new BlockUploadData(new ByteBufferInputStream(dataSize, blockBuffer));
      }
      ...
      /**
       * Provide an input stream from a byte buffer; supporting {@link #mark(int)}, which is
       * required to enable replay of failed PUT attempts.
       */
      class ByteBufferInputStream extends InputStream {
        ...
        /**
         * After the stream is closed, set the local reference to the byte buffer to null; this
         * guarantees that future attempts to use stream methods will fail.
         */
        @Override
      ...
      File destFile =
          getOwner()
              .createTmpFileForWrite(
                  String.format("obs-block-%04d-", index), limit, getOwner().getConf());
      return new DiskBlock(destFile, limit, index);
    }
  }

  /**
   * Stream to a file. This will stop at the limit; the caller is expected to create a new block.
   */
  static class DiskBlock extends DataBlock {
    ...
    DiskBlock(File bufferFile, int limit, long index) throws FileNotFoundException {
      super(index);
      this.limit = limit;
      ...
      switch (state) {
        case Writing:
          if (bufferFile.exists()) {
            ...
            LOG.debug("Block[{}]: Deleting buffer file as upload did not start", index);
            closeBlock();
          }
          break;

        case Upload:
          LOG.debug("Block[{}]: Buffer file {} exists —close upload stream", index, bufferFile);
          break;
    ...
    /**
     * Close the block. This will delete the block's buffer file if the block has not previously
     * been closed.
     */
    void closeBlock() {
