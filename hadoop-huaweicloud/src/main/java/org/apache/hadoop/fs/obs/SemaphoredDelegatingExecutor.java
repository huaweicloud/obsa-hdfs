 ...
 * semaphore. Task submissions require permits, task completions release permits.
 *
 * <p>This is a refactoring of {@link BlockingThreadPoolExecutorService}; that code contains the
 * thread pool logic, whereas this isolates the semaphore and submit logic for use with other thread
 * pools and delegation models. In particular, it <i>permits multiple per stream executors to share
 * a single per-FS-instance executor; the latter to throttle overall load from the the FS, the
 * others to limit the amount of load which a single output stream can generate.</i>
 *
 * <p>This is inspired by <a
...
class SemaphoredDelegatingExecutor extends ForwardingListeningExecutorService {
  ...
  SemaphoredDelegatingExecutor(
      ListeningExecutorService executorDelegatee, int permitCount, boolean fair) {
  ...
  /**
   * Get the number of permits available; guaranteed to be {@code 0 <= availablePermits <= size}.
   *
