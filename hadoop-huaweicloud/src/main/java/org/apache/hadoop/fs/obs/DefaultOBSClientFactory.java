...
class DefaultOBSClientFactory extends Configured implements OBSClientFactory {
    ...
    private static void initConnectionSettings(final Configuration conf,
        ...
        if (!originEndPoint.isEmpty()
            && !originEndPoint.startsWith(OBSConstants.HTTP_PREFIX)
            && !originEndPoint.startsWith(OBSConstants.HTTPS_PREFIX)) {
            String newEndPointWithSchema;
            if (secureConnections) {
                newEndPointWithSchema =
                    OBSConstants.HTTPS_PREFIX + originEndPoint;
            } else {
                newEndPointWithSchema =
                    OBSConstants.HTTP_PREFIX + originEndPoint;
            }
