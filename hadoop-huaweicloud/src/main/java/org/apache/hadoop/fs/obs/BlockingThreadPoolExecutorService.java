...
final class BlockingThreadPoolExecutorService extends SemaphoredDelegatingExecutor {
  ...
  private BlockingThreadPoolExecutorService(
      int permitCount, ThreadPoolExecutor eventProcessingExecutor) {
   ...
   * @return a thread factory that creates named, daemon threads with the supplied exception handler
   *     and normal priority
   */
  ...
  /**
   * A thread pool that that blocks clients submitting additional tasks if there are already {@code
   * activeTasks} running threads and {@code waitingTasks} tasks waiting in its queue.
   *
  ...
  static BlockingThreadPoolExecutorService newInstance(
      int activeTasks, int waitingTasks, long keepAliveTime, TimeUnit unit, String prefixName) {
