...
#### 二.ranger-admin和ranger-usersync安装配置
...
--解压ranger-2.0.0-admin.tar.gz到/home/modules/ranger/ranger-2.0.0-admin

--配置ranger-2.0.0-admin/install.properties文件：如下设置仅供参考，可依据您的自身要求进行相关配置项的调整
...
--安装ranger-2.0.0-admin/setup.sh

--启动ranger-admin start
相关日志存放在ranger-2.0.0-admin/ews/logs目录下

--验证：浏览器打开http://{rangeradmin}:6080/

3.安装ranger-usersync(可选)

--解压ranger-2.0.0-usersync.tar.gz到/home/modules/ranger/ranger-2.0.0-usersync

--配置ranger-2.0.0-usersync/install.properties文件：如下设置仅供参考，可依据您的自身要求进行相关配置项的调整
...
--安装ranger-2.0.0-usersync/setup.sh

--配置/etc/ranger/usersync/conf/ranger-ugsync-site.xml
...
star_rpc_server.sh：mrs集成使用，里面会涉及很多mrs特有的相关信息

start_server.sh：开源大数据集群上面使用
...
| ranger.obs.service.dt.secret.file  | 否 | 空 | 安全集群配置项，ShareFileSecretProvider秘钥提供器加载的秘钥文件在hdfs中的存储路径，如果没有配置秘钥文件位置默认自动生成并存储在hdfs://xxx/${user}/secret.jceks |
...
| ranger.obs.client.impl  | 否 | org.apache.ranger.obs.client.LoadBalanceRangerObsClientImpl | org.apache.ranger.obs.client.LoadBalanceRangerObsClientImpl带负载均衡能力的客户端
和org.apache.ranger.obs.client.RangerObsClientImpl需要依赖外置负载均衡服务的客户端 |
